...
def post_instance_processing(model_instance):
    ...
    discretizer.apply_to(model_instance,nfe=1,wrt=model_instance.DIS,scheme='FORWARD')
    discretizer.apply_to(model_instance,nfe=47,wrt=model_instance.TIME,scheme='BACKWARD')

    # What it should be to match description in paper
    #discretizer.apply_to(model_instance,nfe=48,wrt=model_instance.TIME,scheme='BACKWARD')

    TimeStep = model_instance.TIME[2]-model_instance.TIME[1]

    def supcost_rule(m,k):
        return sum(m.cs*m.s[k,j,t]*(TimeStep) for j in m.SUP for t in m.TIME.get_finite_elements())
    model_instance.supcost = Expression(model_instance.SCEN,rule=supcost_rule)

    def boostcost_rule(m,k):
        return sum(m.ce*m.pow[k,j,t]*(TimeStep) for j in m.LINK_A for t in m.TIME.get_finite_elements())
    model_instance.boostcost = Expression(model_instance.SCEN,rule=boostcost_rule)

    def trackcost_rule(m,k):
        return sum(m.cd*(m.dem[k,j,t]-m.stochd[k,j,t])**2.0 for j in m.DEM for t in m.TIME.get_finite_elements())
    model_instance.trackcost = Expression(model_instance.SCEN,rule=trackcost_rule)

    def sspcost_rule(m,k):
        return sum(m.cT*(m.px[k,i,m.TIME.last(),j]-m.px[k,i,m.TIME.first(),j])**2.0 for i in m.LINK for j in m.DIS)
    model_instance.sspcost = Expression(model_instance.SCEN,rule=sspcost_rule)

    def ssfcost_rule(m,k):
        return sum(m.cT*(m.fx[k,i,m.TIME.last(),j]-m.fx[k,i,m.TIME.first(),j])**2.0 for i in m.LINK for j in m.DIS)
    model_instance.ssfcost = Expression(model_instance.SCEN,rule=ssfcost_rule)

    def cost_rule(m,k):
        return 1e-6*(m.supcost[k] + m.boostcost[k] + m.trackcost[k] + m.sspcost[k] + m.ssfcost[k])
    model_instance.cost = Expression(model_instance.SCEN,rule=cost_rule)

    def mcost_rule(m):
        return (1.0/m.S)*sum(m.cost[k] for k in m.SCEN)
    model_instance.mcost = Expression(rule=mcost_rule)

    def eqcvar_rule(m,k):
        return m.cost[k] - m.nu <= m.phi[k];
    model_instance.eqcvar = Constraint(model_instance.SCEN,rule=eqcvar_rule)

    def obj_rule(m):
        return (1.0-m.cvar_lambda)*m.mcost + m.cvar_lambda*m.cvarcost
    model_instance.obj = Objective(rule=obj_rule)
